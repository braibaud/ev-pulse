...
def save_car(car_data):
    ...
    try:
        ...
        st.success("Thank you for your contribution! It's pending review.")
    ...
    with st.form("contribute_form"):
        for key, value in schema['properties'].items():
            if value['type'] == "string":
                car_data[key] = st.text_input(key.capitalize())
            elif value['type'] == "integer":
                car_data[key] = st.number_input(key.capitalize(), min_value=1886, max_value=2100, step=1)
            elif value['type'] == "number":
                car_data[key] = st.number_input(key.capitalize(), format="%.2f")
            elif value['type'] == "array":
                car_data[key] = st.text_area(key.capitalize()).split(",")
            elif value['type'] == "object":
                for sub_key, sub_value in value['properties'].items():
                    car_data.setdefault(key, {})[sub_key] = st.number_input(f"{key.capitalize()} - {sub_key.capitalize()}", format="%.2f")
